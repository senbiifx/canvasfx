...
class Color {
     ...
     * @return Hue value in the range in the range `0.0-360.0`.
     */
    val hue: Double
        get() = RGBtoHSB(red.toDouble(), green.toDouble(), blue.toDouble())[0]

    /**
     * Gets the saturation component of this `Color`.
     * @return Saturation value in the range in the range `0.0-1.0`.
     */
    val saturation: Double
        get() = RGBtoHSB(red.toDouble(), green.toDouble(), blue.toDouble())[1]
    ...
    val brightness: Double
        get() = RGBtoHSB(red.toDouble(), green.toDouble(), blue.toDouble())[2]
     ...
     */
    fun deriveColor(
        hueShift: Double, saturationFactor: Double,
        brightnessFactor: Double, opacityFactor: Double
    ): Color {
        val hsb = RGBtoHSB(red.toDouble(), green.toDouble(), blue.toDouble())
        ...
        val s = max(min(hsb[1] * saturationFactor, 1.0), 0.0)
        b = max(min(b * brightnessFactor, 1.0), 0.0)
        val a = max(min(opacity * opacityFactor, 1.0), 0.0)
        return hsb(h, s, b, a)
    }
    ...
    fun grayscale(): Color {
        ...
        return color(gray, gray, gray, opacity.toDouble())
    }
     ...
     */
    fun invert(): Color {
        return color(1.0 - red, 1.0 - green, 1.0 - blue, opacity.toDouble())
    }
    ...
    private var platformPaint: Any? = null
    ...
    constructor(
        ...
        require(!(red < 0 || red > 1)) { "Color's red value ($red) must be in the range 0.0-1.0" }
        require(!(green < 0 || green > 1)) { "Color's green value ($green) must be in the range 0.0-1.0" }
        require(!(blue < 0 || blue > 1)) { "Color's blue value ($blue) must be in the range 0.0-1.0" }
        require(!(opacity < 0 || opacity > 1)) { "Color's opacity value ($opacity) must be in the range 0.0-1.0" }
     ...
     * @param opacity opacity ranging from `0` to `1`
     */
    private constructor(red: Float, green: Float, blue: Float) {
        this.red = red
        this.green = green
        this.blue = blue
    }
    ...
    fun interpolate(endValue: Color, t: Double): Color {
        ...
        return Color(
            (
                    red + (endValue.red - red) * ft).toDouble(),
            (
                    green + (endValue.green - green) * ft).toDouble(),
            (
                    blue + (endValue.blue - blue) * ft).toDouble(),
            (
    ...
    override fun hashCode(): Int {
        ...
        val r = round(red * 255.0).toInt()
        val g = round(green * 255.0).toInt()
        val b = round(blue * 255.0).toInt()
        val a = round(opacity * 255.0).toInt()
    ...
    override fun toString(): String {
        ...
        val g = round(green * 255.0).toInt()
        val b = round(blue * 255.0).toInt()
        val o = round(opacity * 255.0).toInt()
        return "St"
    }
    ...
    companion object {
         ...
         */
        private const val DARKER_BRIGHTER_FACTOR = 0.7

        /**
         * Saturation change factor for saturate() and desaturate() methods.
         */
        private const val SATURATE_DESATURATE_FACTOR = 0.7
        ...
        private fun checkRGB(red: Int, green: Int, blue: Int) {
            require(!(red < 0 || red > 255)) { "Color.rgb's red parameter ($red) expects color values 0-255" }
            require(!(green < 0 || green > 255)) { "Color.rgb's green parameter ($green) expects color values 0-255" }
            require(!(blue < 0 || blue > 255)) { "Color.rgb's blue parameter ($blue) expects color values 0-255" }
        }
        /**
         ...
         * <tr class="rowColor">
         * <td class="colFirst">`Color.web("orange");`</td>
         * <td class="colLast">`Color.ORANGE`</td>
        </tr> *
         * <tr class="altColor">
         * <td class="colFirst">`Color.web("0xff668840");`</td>
         * <td class="colLast">`Color.rgb(255, 102, 136, 0.25)`</td>
        </tr> *
         * <tr class="rowColor">
         * <td class="colFirst">`Color.web("0xff6688");`</td>
         * <td class="colLast">`Color.rgb(255, 102, 136, 1.0)`</td>
        </tr> *
         * <tr class="altColor">
         * <td class="colFirst">`Color.web("#ff6688");`</td>
         * <td class="colLast">`Color.rgb(255, 102, 136, 1.0)`</td>
        </tr> *
         * <tr class="rowColor">
         * <td class="colFirst">`Color.web("#f68");`</td>
         * <td class="colLast">`Color.rgb(255, 102, 136, 1.0)`</td>
        </tr> *
         * <tr class="altColor">
         * <td class="colFirst">`Color.web("rgb(255,102,136)");`</td>
         * <td class="colLast">`Color.rgb(255, 102, 136, 1.0)`</td>
        </tr> *
         * <tr class="rowColor">
         * <td class="colFirst">`Color.web("rgb(100%,50%,50%)");`</td>
         * <td class="colLast">`Color.rgb(255, 128, 128, 1.0)`</td>
        </tr> *
         * <tr class="altColor">
         * <td class="colFirst">`Color.web("rgb(255,50%,50%,0.25)");`</td>
         * <td class="colLast">`Color.rgb(255, 128, 128, 0.25)`</td>
        </tr> *
         * <tr class="rowColor">
         * <td class="colFirst">`Color.web("hsl(240,100%,100%)");`</td>
        ...
        fun web(colorString: String?, opacity: Double = 1.0): Color {
            ...
            require(!colorString.isEmpty()) { "Invalid color specification" }
            ...
            if (color.startsWith("#")) {
                ...
                val col = NamedColors.get(color)
                if (col != null) {
                    return if (opacity == 1.0) {
                        col
                    } else {
                        color(col.red.toDouble(), col.green.toDouble(), col.blue.toDouble(), opacity)
                    }
                }
            }
            ...
            try {
                ...
                if (len == 3) {
                    ...
                    return color(r / 15.0, g / 15.0, b / 15.0, opacity)
                } else if (len == 4) {
                    r = color.substring(0, 1).toInt(16)
                    g = color.substring(1, 2).toInt(16)
                    b = color.substring(2, 3).toInt(16)
